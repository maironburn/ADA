...
class Command(BaseCommand):
    help = 'batch process to analyze Elastic and analyze Events backlog'
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            auditprocess.setInitial({"process": Auditprocess.PROCESS_ANALYSIS_BACKLOG, "pid": current_pid})

            auditoriaProcesses=Auditprocess.objects.filter(end_date=None, process=Auditprocess.PROCESS_ANALYSIS_BACKLOG).all()
            ...
            result=AnalysisAveria.analyzeElastic({"analyze_event":True}, auditprocess)
