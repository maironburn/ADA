...
class CustomerValidation:
    ...
    def validate(params):
        ...
        if (validation.code == 0 ):
            validationScripts=Validationscript.objects.filter(belongto='CUSTOMER').filter(status=True).order_by('order').all()
            ...
            for validationScript in validationScripts:
                if (validation.code == 0):
                    ...
                    try:
                        ...
                        class_script_info=(settings.VALIDATION_SCRIPTS_PATH["CUSTOMER"]+"."+validationScript.classname).split(".")
